...
class Agent(BaseModel):
    ...
    def build_dqn(self):
        ...
        initializer = tf.keras.initializers.TruncatedNormal(0, 0.02)  #tf.truncated_normal_initializer(0, 0.02)
        ...
        with tf.name_scope('prediction'):
            ...
            self.q_action = tf.compat.v1.argmax(self.q, dimension=1)

            q_summary = []
            avg_q = tf.reduce_mean(self.q, 0)
            for idx in range(self.env.action_size):
                q_summary.append(tf.summary.histogram('q/%s' % idx, avg_q[idx]))
            self.q_summary = tf.compat.v1.summary.merge(q_summary, 'q_summary')
